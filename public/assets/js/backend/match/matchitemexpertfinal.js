define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    $("#scoringSuccess").click(function(){
        ...
        if(!scoreIsNull){
            Fast.api.ajax({
                url:'match/matchitemexpertfinal/scoring',
    ...
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: 'match/matchitemexpertfinal/index' + location.search,
                    add_url: 'match/matchitemexpertfinal/add',
                    edit_url: 'match/matchitemexpertfinal/edit',
                    del_url: 'match/matchitemexpertfinal/del',
                    multi_url: 'match/matchitemexpertfinal/multi',
                    table: 'match_item'
                }
            });
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        {field: 'final_status', title: __('Score_count'),searchList:{" ":'未评分',"Y":'已评分'},formatter:function(index,value){
                            if(value.is_score == 'Y'){
            ...
            events:{
                operate:{
                    'click .btn-viewDetail': function (e, value, row, index) {
                        ...
                        Fast.api.open("match/matchitemexpertfinal/audit_view?id="+row.item_id+"&type=viewDetail", __('查看'),{area: ['100%', '100%']})
                    },
                    'click .btn-scoring': function (e, value, row, index) {
                        ...
                        var table = $(that).closest('table');
                        var options = table.bootstrapTable('getOptions');
                        Fast.api.open("match/matchitemexpertfinal/scoring_view?id="+row.item_id, __('评分'),{
                            area: ['100%', '100%'],
