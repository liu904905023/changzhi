 ...
 * @icon fa fa-circle-o
 */
class Matchitemschool extends Backend
{
    ...
    protected $searchFields = 'item_id';
    ...
    public function index()
    {
        ...
        $id = $this->auth->id;
        //判断当前登录管理员的角色(学校管理员:查看自己学校的数据，平台管理员：查看全部数据)
        //获取登陆者权限
        $auth_group = $adminModel->field("g.id")
                                    ->alias("a")
                                    ->join("auth_group_access s","s.uid = a.id","left")
                                    ->join("auth_group g","g.id = s.group_id","left")
                                    ->where("a.id","eq",$id)
                                    ->select();
        $auth_group_id = '';
        if(!empty($auth_group[0]['id'])){
            $auth_group_id = $auth_group[0]['id'];
        }
        //获取当前登录管理员的school_id
        ...
        $sort = ['matchitem.first_score'=>'desc','matchitem.item_id'=>'desc'];

        if($auth_group_id == '3'){
            //学校管理员
            $total = $this->model
                ->alias("matchitem")
                ->field("matchitem.item_name,matchitem.category,matchitem.first_score,matchitem.half_score,matchitem.final_score,matchitem.audit_status,matchitem.item_status,ms.school_name,matchitem.item_id,mg.group_name,mt.track_name,u.nickname,u.major,u.education,u.college,GROUP_CONCAT(distinct t.teacher_name) teacher_name,COUNT(s.score_id) AS expertCount,COUNT(s.score)AS scoreCount")
                ->where($where)
                ->where("ms.school_id","eq",$school_id)
                ...
                ->join("match_item_teacher t","t.item_id = matchitem.item_id","left")
            ...
            $list = $this->model
                ...
                ->limit($offset, $limit)
                ->select();
        }else if($auth_group_id == '2'){
            //平台管理员
            $total = $this->model
                ->alias("matchitem")
                ->field("matchitem.item_name,matchitem.category,matchitem.first_score,matchitem.half_score,matchitem.final_score,matchitem.audit_status,matchitem.item_status,ms.school_name,matchitem.item_id,mg.group_name,mt.track_name,u.nickname,u.major,u.education,u.college,COUNT(s.score_id) AS expertCount,COUNT(s.score)AS scoreCount")
                ->where($where)
                ->join("match m","m.match_id = matchitem.match_id","left")
                ->join("match_group mg","mg.group_id = matchitem.group_id","left")
                ->join("match_track mt","mt.track_id = matchitem.track_id","left")
        ...
        $this->assignconfig("auth_group_id",$auth_group_id);
        $this->assignconfig("school_id",$school_id);
    ...
    public function firstView(){
        ...
        $this->view->assign("teacherList", $teacherList);
        $this->view->assign("item_id", $id);
        $this->view->assign("firstScore", $row[0]['first_score']);
        return $this->view->fetch("firstScore");
    }
    ...
    public function firstScore(){
        ...
        $arr['first_score'] = $this->request->post("firstScore");
        if($arr['first_score'] == ''){
            $arr['first_score'] = null;
        }
        $arr['item_id'] = $this->request->post("item_id");
        $matchItemModel->isUpdate(true)->save($arr);
        $this->success("评分成功！");
    }
